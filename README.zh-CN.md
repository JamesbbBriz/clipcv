...
目前尚未发布任何 tag。一旦推送第一个 `v*` tag，[release 工作流](./.github/workflows/release.yml) 会自动把签名好的 `.crx` 和 `.zip` 挂到 [Releases](https://github.com/JamesbbBriz/clipcv/releases) 页面 —— 开启开发者模式后把 `.crx` 拖入 `chrome://extensions` 即可安装。在此之前请走下面的开发者模式路径。
...
| | |
|---|---|
| 源码版本 | `0.1.0`（15 个故事的 MVP，尚未打 tag） |
| Releases | 尚未推送 tag —— 请用 `npm run build` + unpacked 模式安装 |
| Manifest | V3 |
| 最低 Chrome 版本 | 116 |
| 测试 | 74 / 74 通过 |
